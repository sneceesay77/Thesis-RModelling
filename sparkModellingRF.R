...
allDataOriginal <- read.table(file = "rf.report", header = TRUE)#contains both 128
...
#Distribution plot
ne1 <- generateDistributionPlot(allDataOriginal, "DataSizeMB", 1, "1MB", "NumEx")
ne2 <- generateDistributionPlot(allDataOriginal, "DataSizeMB", 15, "15MB", "NumEx")  
ne3 <- generateDistributionPlot(allDataOriginal, "DataSizeMB", 76, "76MB", "NumEx")
ne4 <- generateDistributionPlot(allDataOriginal, "DataSizeMB", 1908, "1908MB", "NumEx")
ne5 <- generateDistributionPlot(allDataOriginal, "DataSizeMB", 4006, "4006MB", "NumEx")
ne6 <- generateDistributionPlot(allDataOriginal, "DataSizeMB", 6867, "6867MB", "NumEx")
annotate_figure(ggarrange(ne1,ne3,ne5,ne6, ncol=2, nrow=2, common.legend = TRUE, legend = "bottom"), top="SVM Algorithm: Scatter Plot of scaling Executors")
...
rf.tune.grid.rf <- train(Duration.s.~ DataSizeGB + NumEx  + ExCore + ExMem, data=training_set[c(-6)], method="rf", trControl=control)
svm.tune.grid.rf <- train(Duration.s.~ DataSizeGB + NumEx  + ExCore + ExMem, data=training_set[c(-6)], method="svmRadial", tuneGrid=grid_radial, trControl=control)
...
set.seed(123453)
testsamplerf <- (sample_n(test_set, 100))

tp1rf <- generateModelPlotOnTest(testsamplerf, svm, "SVM", "Index", "Time(s)")
tp2rf <- generateModelPlotOnTest(testsamplerf, svm.tune.random, "Random Forest", "Observation", "Time(s)")
tp3rf <- generateModelPlotOnTest(testsamplerf, svm.tune.grid, "SVM Grid", "Index", "Time(s)")

tp1rf
tp2rf
tp3rf

ggarrange(tp2kmeans, tp2lr, tp2svm, tp2lir, tp2rf, nrow=3, ncol = 2, common.legend = TRUE, legend = "bottom")
...
save(rf.tune.grid.rf, file = "RFGridModelForRF.RData")
save(svm.tune.grid.rf, file = "SVMGridModelForRF.RData")

...
allDataOriginal <- read.table(file = "compression.report", header = TRUE)#contains both 128
...
allDataOriginal <- select(allDataOriginal, Duration.s., NumEx, ExCore, ExMem, LevelPar, DataSizeMB, DataSizeGB, LevelPar, CompAlgo, sh.com, spill.com, broa.comp, observations, features)
...
p01 <- generatePlot(allDataOriginal, "DataSizeMB", 7661, "7GB", "CompAlgo")
p02 <- generatePlot(allDataOriginal, "DataSizeMB", 11491, "11GB", "CompAlgo")  
...
annotate_figure(ggarrange(p01,p02, ncol=1, nrow=2, common.legend = TRUE, legend = "bottom"), top="KMeans Compression Algorithm")

...
allDataOriginal <- read.table(file = "lr.report", header = TRUE)#contains both 128
...
lr.svm.tune.random <- train(Duration.s.~ DataSizeGB + NumEx  + ExCore + ExMem, data=training_set[c(-6)], method="svmRadial", trControl=control)
...
rf.tune.grid.lr <- train(Duration.s.~ DataSizeGB + NumEx  + ExCore + ExMem, data=training_set[c(-6)], method="rf", trControl=control)
svm.tune.grid.lr <- train(Duration.s.~ DataSizeGB + NumEx  + ExCore + ExMem, data=training_set[c(-6)], method="svmRadial", tuneGrid=grid_radial, trControl=control)
...
set.seed(12340002)
testsamplelr <- (sample_n(test_set, 100))

tp1lr <- generateModelPlotOnTest(testsamplelr, svm, "SVM", "Index", "Time(s)")
tp2lr <- generateModelPlotOnTest(testsamplelr, lr.svm.tune.random, "Logistic Regression", "Observation", "Time(s)")
tp3lr <- generateModelPlotOnTest(testsamplelr, svm.tune.grid, "SVM Grid", "Index", "Time(s)")

tp1lr
tp2lr
tp3lr

ggarrange(tp2kmeans, tp2lr, tp2svm, tp2lir, nrow=2, ncol = 2, common.legend = TRUE, legend = "bottom")
...
save(rf.tune.grid.lr, file = "RFGridModelForLR.RData")
save(svm.tune.grid.lr, file = "SVMGridModelForLR.RData")

...
p <- generatePlotBestConfig("SVM.txt", "Title", "DataSize", 39)

p$minob
p$maxob
p$p

generatePlotBestDeadline <- function(datafile, title, color, ds, time){
  
  f <- read.table(file = datafile, header = TRUE, fill = TRUE)
  f <- na.omit(f)
  f <- filter(f, f$DataSize==ds & f$Prediction <= time)
...
p <- generatePlotBestDeadline("SVMORI.txt", "Title", "DataSize", 20, 600)

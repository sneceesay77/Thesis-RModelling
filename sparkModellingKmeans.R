...
kmeans.svm.tune.random <- train(Duration.s.~ DataSizeGB + NumEx  + ExCore + ExMem, data=training_set[c(-6)], method="svmRadial", trControl=control)
...
rf.tune.grid.kmeans <- train(Duration.s.~ DataSizeGB + NumEx  + ExCore + ExMem, data=training_set[c(-6)], method="rf", trControl=control)
svm.tune.grid.kmeans <- train(Duration.s.~ DataSizeGB + NumEx  + ExCore + ExMem, data=training_set[c(-6)], method="svmRadial", tuneGrid=grid_radial, trControl=control)
...
# df
set.seed(445454)
testsamplekmeans <- (sample_n(test_set, 100))

tp1kmeans <- generateModelPlotOnTest(testsamplekmeans, svm, "SVM", "Index", "Time(s)")
tp2kmeans <- generateModelPlotOnTest(testsamplekmeans, kmeans.svm.tune.random, "K-means", "Observation", "Time(s)")
tp3kmeans <- generateModelPlotOnTest(testsamplekmeans, svm.tune.grid, "SVM Grid", "Index", "Time(s)")
...
tp5 <- generateModelPlotOnTest(sample_n(test_set, 60), rf.tune.random, "RF Random", "Index", "Time(s)")
...
save(rf.tune.grid.kmeans, file = "RFGridModelForKMEANS.RData")
save(svm.tune.grid.kmeans, file = "SVMGridModelForKMEANS.RData")

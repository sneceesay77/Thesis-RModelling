...
allDataOriginal <- read.table(file = "linear.report", header = TRUE)#contains both 128
...
lm.svm.tune.random <- train(Duration.s.~ DataSizeGB + NumEx  + ExCore + ExMem, data=training_set[c(-6)], method="svmRadial", trControl=control)
...
# testsample <- sample_n(test_set, 10)
# testdur <- testsample$Duration.s.
# testdata <- testsample$DataSizeMB
# predictsample <- predict(svm, testsample)
# testerror <- ((testdur - predictsample)/testdur)*100
# 
# df <- data.frame(testdur, predictsample, testdata, testerror)
# df

set.seed(1023000)
testsamplelir <- (sample_n(test_set, 100))
tp1lir <- generateModelPlotOnTest(testsamplelir, svm, "SVM", "Index", "Time(s)")
tp2lir <- generateModelPlotOnTest(testsamplelir, lm.svm.tune.random, "Linear Regression", "Observation", "Time(s)")
tp3lir <- generateModelPlotOnTest(testsamplelir, svm.tune.grid, "SVM Grid", "Index", "Time(s)")
